...
exports.handler = async function(event) {
    ...
    if (event.httpMethod === 'POST') {
        try {
            ...
            if (rateData.count >= 5) {
                return { statusCode: 429, headers: CORS, body: JSON.stringify({ error: 'Previse poruka. Pricekajte sat vremena.' }) };
            }
            rateData.count++;
            ...
            if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
                return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Unesite valjanu email adresu' }) };
            }
            var subject = esc((body.subject || '').substring(0, 100));
            var message = esc((body.message || '').substring(0, 2000));
            if (!message) {
                return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Unesite poruku' }) };
            }
